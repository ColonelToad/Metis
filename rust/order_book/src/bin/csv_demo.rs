...
fn main() -> anyhow::Result<()> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--signal" => {
                i += 1;
                if i < args.len() {
                    signal_col = Some(args[i].clone());
                }
            }
            "--price" => {
                i += 1;
                if i < args.len() {
                    price_col = Some(args[i].clone());
                }
            }
            "--qty" => {
                i += 1;
                if i < args.len() {
                    qty = args[i].parse().unwrap_or(1.0);
                }
            }
            "--tick" => {
                i += 1;
                if i < args.len() {
                    tick = args[i].parse().unwrap_or(0.001);
                }
            }
            "--thr" => {
                i += 1;
                if i < args.len() {
                    thr = args[i].parse().unwrap_or(0.0);
                }
            }
            _ => {}
        }
    ...
    let features_path = match features_path {
        Some(p) => p,
        None => {
            usage();
            return Err(anyhow::anyhow!("missing --features"));
        }
    };
    ...
    let price_colname = price_col.unwrap_or_else(|| {
        ...
        cands
            .iter()
            .find(|c| headers.iter().any(|h| h.eq_ignore_ascii_case(c)))
            .unwrap_or(&"close")
            .to_string()
    });
    ...
    let price_idx = headers
        .iter()
        .position(|h| h.eq_ignore_ascii_case(&price_colname))
        .ok_or_else(|| anyhow::anyhow!(format!("price column '{}' not found", price_colname)))?;
    let signal_idx = headers
        .iter()
        .position(|h| h.eq_ignore_ascii_case(&signal_colname));
    ...
    for rec in rdr.records() {
        ...
        let mid = match (ob.best_bid(), ob.best_ask()) {
            (Some(bb), Some(ba)) => (bb + ba) / 2.0,
            _ => f64::NAN,
        };
        ...
        if sig_val > thr {
            ...
            for t in ob.trades.iter().rev() {
                if t.buy_id == my_id {
                    pnl.on_trade(Side::Buy, t.price, t.qty);
                } else {
                    break;
                }
            }
            ...
            for t in ob.trades.iter().rev() {
                if t.sell_id == my_id {
                    pnl.on_trade(Side::Sell, t.price, t.qty);
                } else {
                    break;
                }
            }
    ...
    // summary
    println!(
        "trades={}, final_pos={:.3}, cash={:.2}, equity={:.2}",
        ob.trades.len(),
        pnl.pos,
        pnl.cash,
        pnl.equity
