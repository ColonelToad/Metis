...
//!
//! This is the deterministic half of a framework that also exists,
//! independently, as an LLM-prompted JSON schema in the rag crate
//! (`reasoning_chain.rs` / `explanation_parser.rs`). The intended shape
//! going forward: this module computes the real numbers (Bayesian update,
//! expected value, risk score), and the RAG/LLM layer turns already-computed,
//! auditable numbers into a natural-language explanation, rather than being
//! asked to reconstruct the whole framework from scratch. That wiring hasn't
//! happened yet — this module isn't called from anywhere in this crate
//! yet either — but this is where it's happening from once it does.
