...
impl CsvTickParser {
    pub fn parse_file(path: &Path) -> Result<Vec<MarketEvent>> {
        ...
        for (i, result) in reader.records().enumerate() {
            ...
            if record.len() < 6 {
                warn!("Skipping row {}: expected at least 6 columns, got {}", i + 2, record.len());
                continue;
            }
            let timestamp: DateTime<chrono::Utc> = match record[0].parse() {
                ...
                Err(e1) => {
                    ...
                    match chrono::NaiveDateTime::parse_from_str(&record[0], "%Y-%m-%d %H:%M:%S") {
                        Ok(naive) => {
                            let t = chrono::DateTime::<chrono::Utc>::from_utc(naive, chrono::Utc);
                            debug!("Row {}: Parsed timestamp (naive fallback): {}", i + 2, t);
                            t
                        },
                        Err(e2) => {
                            warn!("Skipping row {}: invalid timestamp: {} | fallback error: {}", i + 2, e1, e2);
                            continue;
                        }
                    }
                }
