...
impl TwapExecutor {
    ...
    pub fn generate_child_orders(&self, parent: &ParentOrder) -> Result<Vec<ChildOrder>> {
        ...
        let num_slices = (total_duration.num_seconds() / self.slice_duration.num_seconds()) as usize;
    ...
    pub fn calculate_metrics(&self, parent: &ParentOrder, fills: &[Fill], benchmark_price: f64) -> ExecutionMetrics {
        ...
        let slippage = match parent.side {
            Side::Bid => benchmark_price - avg_fill_price,  // Lower is better for buys
            Side::Ask => avg_fill_price - benchmark_price,  // Higher is better for sells
        };

        let slippage_bps = (slippage / benchmark_price).abs() * 10000.0;
